...
import { createButton, createInput, showToast } from '../utils/UIKit.js'
...
export class Gallery {
  ...
  private async showEmailModal(sessionId: string) {
    ...
    Object.assign(modalBox.style, {
      ...
      width: '90%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '0.5rem',
      boxShadow: '0 10px 25px rgba(0,0,0,0.5)'
    })

    const title = document.createElement('h3')
    title.textContent = 'Send the photo via email'
    title.style.margin = '0'
    title.style.fontSize = '1.25rem'

    const desc = document.createElement('p')
    desc.textContent = 'Enter your email address to receive your photos.'
    ...
    Object.assign(btnGroup.style, { display: 'flex', gap: '1rem', marginTop: '0.5rem' })
    ...
    const submit = async () => {
      ...
      showToast('Sending email...', 'info', 0, toastId)
      ...
      try {
        ...
        showToast('Email sent successfully!', 'success', 3000, toastId)
      } catch (err: any) {
        showToast(err.message || 'Failed to send email', 'error', 4000, toastId)
      }
    }
