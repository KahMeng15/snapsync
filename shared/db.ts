...
try { db.exec(`ALTER TABLE events ADD COLUMN photo_count INTEGER NOT NULL DEFAULT 4`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN countdown INTEGER NOT NULL DEFAULT 5`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN capture_interval INTEGER NOT NULL DEFAULT 1`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN post_capture_preview INTEGER NOT NULL DEFAULT 2`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN dslr_iso TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN dslr_shutterspeed TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN dslr_aperture TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN dslr_focus_mode TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }

try { db.exec(`ALTER TABLE global_settings ADD COLUMN dslr_aperture TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE global_settings ADD COLUMN dslr_focus_mode TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }

try { db.exec(`ALTER TABLE events ADD COLUMN dslr_whitebalance TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE global_settings ADD COLUMN dslr_whitebalance TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE camera_settings ADD COLUMN dslr_whitebalance TEXT NOT NULL DEFAULT 'auto'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }

try { db.exec(`ALTER TABLE events ADD COLUMN dslr_whitebalance_kelvin INTEGER NOT NULL DEFAULT 5200`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }

try { db.exec(`ALTER TABLE events ADD COLUMN obfuscate_links INTEGER NOT NULL DEFAULT 0`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN expiry_type TEXT NOT NULL DEFAULT 'none'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN expiry_value TEXT NOT NULL DEFAULT ''`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN organizer TEXT NOT NULL DEFAULT ''`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN contact_info TEXT NOT NULL DEFAULT ''`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE events ADD COLUMN operator_password TEXT`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }


try { db.exec(`ALTER TABLE global_settings ADD COLUMN dslr_whitebalance_kelvin INTEGER NOT NULL DEFAULT 5200`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE global_settings ADD COLUMN organizer TEXT NOT NULL DEFAULT ''`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE global_settings ADD COLUMN contact_info TEXT NOT NULL DEFAULT ''`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }

// Phase 2: Rate Limits & Bandwidth
try { db.exec(`ALTER TABLE global_settings ADD COLUMN api_rate_limit_admin INTEGER NOT NULL DEFAULT 500`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE global_settings ADD COLUMN api_rate_limit_share INTEGER NOT NULL DEFAULT 300`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
...
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN archived INTEGER NOT NULL DEFAULT 0`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN share_id TEXT`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN upload_status TEXT NOT NULL DEFAULT 'reserved'`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN upload_started_at INTEGER`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN upload_completed_at INTEGER`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
try { db.exec(`ALTER TABLE photo_sessions ADD COLUMN upload_size_bytes INTEGER`) } catch (e: any) { if (!e.message.includes("duplicate column name")) throw e; }
